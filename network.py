...
def send_openai_request(api_url: str, api_key: str, model_id: str, prompt: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Таймаут при запросе к OpenAI API")
        return None
...
def send_deepseek_request(api_url: str, api_key: str, model_id: str, prompt: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Таймаут при запросе к DeepSeek API")
        return None
    except requests.exceptions.RequestException as e:
...
def send_groq_request(api_url: str, api_key: str, model_id: str, prompt: str) -> Optional[str]:
    ...
    try:
        ...
        logger.error(f"Таймаут при запросе к Groq API")
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"Ошибка при запросе к Groq API: {e}")
        return None
    except Exception as e:
        logger.error(f"Неожиданная ошибка при запросе к Groq API: {e}")
        return None
